    ...
    <div class="col-md-8 offset-md-2">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <p><strong>Tanggal :</strong> {{ $pelatihan->tanggal ?? 'N/A' }}</p>
                <p><strong>Tanggal Akhir :</strong> {{ $pelatihan->tanggal_akhir ?? 'N/A' }}</p>
                <p><strong>Kuota :</strong> {{ $pelatihan->kuota ?? 'N/A' }}</p>
                <p><strong>Biaya :</strong> {{ $pelatihan->biaya ?? 'N/A' }}</p>
                <p><strong>Periode:</strong> {{ $pelatihan->periode ?? 'N/A' }}</p>
            </div>
            <div class="card-footer">
                <a href="{{ route('informasi_dosen', ['id' => $bidang->bidang_id, 'id_dosen' => $dosenBidang->dosen2->dosen_id]) }}" class="btn btn-secondary">Kembali</a>
            </div>
        </div>
    </div>
</div>
