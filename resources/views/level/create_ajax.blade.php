    ...
    $(document).ready(function() {
        $("#form-tambah-level").validate({
            ...
            submitHandler: function(form) {
                $.ajax({
                    url: "{{ url('/level/ajax') }}", // URL untuk request
                    type: "POST", // Metode HTTP dipaksa POST
                    data: $(form).serialize(), // Serialisasi form data
                    success: function(response) {
                        if (response.status) {
                            ...
                            dataLevel.ajax.reload(); // Reload tabel jika perlu
                        } else {
                            $('.error-text').text('');
