...
@else
    <form action="{{ url('/kompetensi_prodi/' . $kompetensi_prodi->prodi_id . '/delete_ajax') }}" method="POST" id="form-delete">
        @csrf
        @method('DELETE')
        <div id="modal-master" class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Hapus Data Kompetensi Prodi</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.status) {
                                ...
                                datakompetensi_prodi.ajax.reload();
                            } else {
                                $('.error-text').text('');
